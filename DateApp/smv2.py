# -*- coding: utf-8 -*-
"""
Created on Thu Oct  6 21:12:30 2022

@author: Nadeeya Norjemee
"""

import pandas as pd
from sklearn.model_selection import train_test_split


df = pd.read_excel('svm_imp_feat_unbal.xlsx')
#df['Class'].replace(['ROTANA', 'BERHI', 'SOGAY', 'IRAQI' , 'DEGLET', 'DOKOL' , 'SAFAVI'],[0, 1 , 2, 3, 4, 5, 6], inplace=True)
#df.info()


x = df.iloc[:, :9]
y = df.iloc[:, 9]

y

from sklearn.preprocessing import StandardScaler
sc = StandardScaler()

x_train , x_test , y_train , y_test = train_test_split(x , y , test_size=0.3, random_state=0)

x_train = sc.fit_transform(x_train)
x_test = sc.fit_transform(x_test)

from sklearn.svm import SVC

svm = SVC(kernel='linear', C=10, gamma=1)
svm.fit(x_train, y_train)

from sklearn.metrics import accuracy_score
y_pred = svm.predict(x_train)
print("Training Accuracy :" , accuracy_score(y_train, y_pred)*100)

y_pred_test = svm.predict(x_test)
print("Testing Accuracy :" , accuracy_score(y_test, y_pred_test)*100)

import seaborn as sns 
import numpy as np 
import matplotlib.pyplot as plt 
from sklearn.metrics import confusion_matrix, accuracy_score 
cm  = confusion_matrix(y_test, y_pred_test) 
cm_df = pd.DataFrame(cm,index = np.unique(y), columns =np.unique(y)) 
plt.subplots(figsize=(20,15)) 
sns.heatmap(cm_df, annot=True, 
cmap="YlGnBu",fmt=",",annot_kws={"size": 13}) 
plt.title('SVM important features Algorithm \nAccuracy:{0:.3f}'.format(accuracy_score(y_test, y_pred_test))) 
plt.ylabel('y_test') 
plt.xlabel('y_test_pred') 
plt.show() 

print(df.head())
"""
from sklearn.model_selection import GridSearchCV
from sklearn.metrics import classification_report

param_grid = {'C' : [0.1, 1, 10 , 100, 1000],
              'gamma':[1,0.1, 0.01 , 0.001, 0.0001],
              'kernel':['rbf', 'linear']}

grid = GridSearchCV(SVC() ,param_grid, refit=True, verbose=3)

grid.fit(x_train, y_train)

#fitting the best model for grid search
print(grid.best_params_)

#print how our model looks after hyper- parameter tuning
print(grid.best_estimator_)


"""
from sklearn.metrics import classification_report
grid_predictions = svm.predict(x_test)

#print classification report
print(classification_report(y_test, grid_predictions))

"""
from sklearn import metrics
print("Test data performance")
print("R squared for test data :", metrics.r2_score(y_test,y_pred_test))
#print("Root mean squared error:", )

RSV = metrics.r2_score(y_test, y_pred_test)
print('R-Squared-value:',RSV)

MAE = metrics.mean_absolute_error(y_test, y_pred_test)
MSE = metrics.mean_squared_error(y_test, y_pred_test)
RMSE = np.sqrt(metrics.mean_squared_error(y_test, y_pred_test))
#display adjusted R-squared
print()
AdjRSV = 1 - (1-RSV)*(len(y_test)-1)/(len(y_test)-x_test.shape[1]-1)
residual = (y_test - y_pred_test)
print('Mean Absolute Error:', MAE)
print('Mean Squared Error:', MSE)
print('Root Mean Squared Error:', RMSE)
print('R-Squared-value:',RSV)
print('Adjusted R-Squared-value:',AdjRSV)
print('statistics of model residuals:')
residual.describe()
"""
